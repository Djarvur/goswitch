...
1. **Семантика D-24 «нечего конвертировать» и якорь для однородного диапазона (A6).**
   ...
   - RESOLVED (2026-09-15): рекомендация (a)–(d) принята целиком — уточнение D-22 в 03-CONTEXT.md: якорь управляет ТОЛЬКО смешанными диапазонами, однородный конвертируется целиком (матрица v1 неприкосновенна). Закреплено золотым корпусом `ConvertRuns` в плане 03-01 Task 2; владелец подтверждает корпус на verify-гейте.
2. **Фактическое поведение замены выделения per-surface (A1/A2).**
   ...
   - RESOLVED (план фазы 3): spike-задача 03-03 Task 1 (кейс select-smoke, живой стол) пинирует фактическую ступень замены и поведение якоря per-surface ДО встраивания логики; вердикты (спайк-таблица в SUMMARY) переносятся в поля expect_sel_step матрицы v2 (план 03-07).
3. **Доставка Super+Буква и матрица перехваченных GNOME комбинаций (A7).**
   ...
   - RESOLVED (план фазы 3): spike-задача 03-05 Task 1 (кейс macr-probe) — живая ProcessKeyEvent-трасса ДО реализации MACR; вердикт определяет дефолт macr.letters для живого кейса и жизнеспособность consumed-upstream детекта (ADR-005 b.2).
4. **Формат биндингов в YAML (дискреция).**
   ...
   - RESOLVED (план фазы 3, 03-02 config_schema): имена-строки нижним регистром (дефолты `shift_r`, `shift+ctrl_r`; комбо через `+`), резолв закрытой таблицей `internal/hotkey/names.go` + `ParseBinding` (keyval/mask с провенансом ibuskeysyms.h); неизвестное имя = невалидный конфиг целиком (D-33); секции ровно hotkeys/timeouts/correction/macr (D-31), yaml-теги snake_case.
5. **CorrectNow — состав принудительной коррекции.**
   ...
   - RESOLVED (план фазы 3, 03-06): минимум — слово (семантика двойного тапа), `CorrectNow() (string, *dbus.Error)` без scope-аргумента; расширение `word|phrase` остаётся дешёвым будущим методом и в объём INST-02 не входит.
