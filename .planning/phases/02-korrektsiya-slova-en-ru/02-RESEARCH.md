...
> Все четыре вопроса закрыты при планировании Фазы 2 (набор планов 02-01..02-07, ревизия 2026-09-11). Распоряжения ниже указывают решающий план/задачу.
...
1. **Триггер внутреннего флипа в Фазе 2** (главный вопрос планирования)
   ...
   - **RESOLVED:** проводка FSM `Single`→флип входит в Фазу 2 — план 02-04, Task 1 (флип-режим; распоряжение зафиксировано в assumptions плана 02-04 как «research A2 → решено планировщиком»).
2. **gnome-text-editor в матрице v1?**
   ...
   - **RESOLVED:** gnome-text-editor добавлен третьей поверхностью — драйвер+smoke-кейс в плане 02-02 (Task 2), живые кейсы со всеми ТРЕМЯ регистрами критерия 1 (word-gte / word-gte-upper / word-gte-capitalized) в плане 02-06 (Task 2); замена формулировки критерия через D-17 не потребовалась.
3. **Судьба буфера после удачной коррекции**
   ...
   - **RESOLVED:** ReplaceToken — буфер хранит исправленный токен (toggle, инвариант «буфер == текст перед курсором»): корпус TestBuffer_ReplaceTokenToggle в плане 02-01 (Task 1), проводка в конвейере — план 02-03 (Task 2, TestActor_ToggleRepeat).
4. **Таймаут ожидания SetSurroundingText после RequireSurroundingText**
   ...
   - **RESOLVED:** verifyWait = 100 мс (константа актёра), при истечении — тихий отказ reason:"verify-timeout" — план 02-03 (Task 1, TestActor_VerifyPaths); латентности фиксируются debug-записью коррекции.
